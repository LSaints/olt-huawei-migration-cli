...
from textual.widgets import DataTable, Static, Button, Input, Header, Footer, Log
from .widgets import input_gpon_origem, ont_status_select, btn_listar, btn_migrar, input_gpon_migrar, input_vlan_migrar

from core.domain.ont import Ont
from core.services import OltServices
...
class TerminalInterface(App):
    ...
    async def __migrar_onts(self, gpon_destino: str, vlan_destino: str):
        ...
        await self.__olt_services.migrar_onts(
            onts=onts,
    ...
    async def __load_data(self, gpon_origem: str, status_ont: str) -> None:
        onts_result = await self.__olt_services.listar_onts(status=status_ont, gpon=gpon_origem)
